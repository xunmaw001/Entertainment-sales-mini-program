		...
		<view class="list b-b" v-for="(item, index) in list" :key="index" @click="onSelectTap(item)">
			<view class="wrapper">
				<view class="address-box">
					<text v-if="item.isdefaultTypes==2" class="tag">默认</text>
					<text class="address">{{item.addressDizhi}}</text>
				</view>
				<view class="u-box">
...
<script>
	export default {
		...
		async onLoad(option) {
			...
			this.user = res.data;
			res = await this.$api.page('address', {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							res = await _this.$api.page('address', {
